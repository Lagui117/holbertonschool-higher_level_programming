...
Endpoints:
  ...
  (other)   -> 404 text/plain: "Endpoint not found"
"""
...
class SimpleAPIHandler(BaseHTTPRequestHandler):
    def _send_json(self, obj: dict, status: int = 200) -> None:
        ...
        # Checker requires EXACT header without charset
    ...
    def do_GET(self):
        if self.path == "/":
            ...
            self._send_json({"name": "John", "age": 30, "city": "New York"}, 200)
        elif self.path == "/status":
            self._send_text("OK", 200)
        else:
            # Checker expects plain text body exactly:
